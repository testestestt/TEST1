...
function ObjectControls(camera, domElement, objectToMove) {
    ...
    /**
     * setAutoRotateSpeed
     * @param {number} newAutoRotateSpeed - (1 == fast)  (0.001 == slow)
    **/
     this.setSpeedAutoRotate = function (newRotationSpeed) {
        autoRotateSpeed = newRotationSpeed;
    };
    ...
    function rotateVertical(deltaMove, mesh) {
      ...
      velocity.y = deltaMove.y * rotationSpeed * 0.1;
    }
    ...
    function rotateVerticalTouch(deltaMove, mesh) {
      ...
      velocity.y = deltaMove.y * rotationSpeedTouchDevices * 0.13;
    }
    ...
    function rotateHorizontal(deltaMove, mesh) {
      ...
      velocity.x = deltaMove.x * rotationSpeed * 0.1;
      lastXDirection = Math.sign(deltaMove.x);
    }
    ...
    function rotateHorizontalTouch(deltaMove, mesh) {
        ...
        velocity.x = deltaMove.x * rotationSpeedTouchDevices * 0.13;
        lastXDirection = Math.sign(deltaMove.x);
    }

    function applyInertia(mesh) {
      if (velocity.x !== 0 || velocity.y !== 0) {
        ...
        gsap.to(mesh.rotation, {
          duration: 0.5,
          y: mesh.rotation.y + velocity.x,
          x: mesh.rotation.x + velocity.y,
          ease: "expo.out"
        });
        ...
        // Detener la rotación si la velocidad es muy baja
        if (Math.abs(velocity.x) < 0.001) velocity.x = 0;
        if (Math.abs(velocity.y) < 0.001) velocity.y = 0;
      }
  
      requestAnimationFrame(() => applyInertia(mesh));
    }
    ...
    function autoRotateObject(mesh) {
      if (!isDragging && autoRotate) {
        if(!firstTouch){
          velocity.x += 1 * autoRotateSpeed * 0.011; 
        }
        else{
          velocity.x += lastXDirection * autoRotateSpeed * 0.011; 
        }
      }
      requestAnimationFrame(() => autoRotateObject(mesh));
    }
